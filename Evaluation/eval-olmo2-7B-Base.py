...
for abbr, path in Settings:  ## classic 4096
    models.append(
        dict(
            ...
            path=path,
            model_kwargs=dict(
                tensor_parallel_size=8,      # 对应原来的 tp=8
                gpu_memory_utilization=0.9,  # 显存占用比例，防止 OOM
                max_model_len=4096+ws,          # 对应 session_len 或 max_seq_len
                trust_remote_code=True,      # 通常需要开启
            ),
            generation_kwargs=dict(
                temperature=0.7,
                top_k=20,
                top_p=0.8,
                max_tokens=ws,               # vLLM 中通常用 max_tokens 限制生成长度
                # mini_p 在 vLLM 最新版中支持，旧版可能不支持，视版本而定
            ),
            max_seq_len=4096, 
            max_out_len=ws, 
            batch_size=4096, 
