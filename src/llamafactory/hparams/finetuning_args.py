...
class FinetuningArguments(
    ...
    prob_threshold: list[float] = field(
    default_factory=list,
    metadata={"help": "Probability threshold for the threshold."}
    )
    threshold_direction: str = field(
    default="None",
    metadata={"help": "Direction of the threshold."}
    )
    def __post_init__(self):
        ...
        # if self.entropy_ignore_percentage is not None and not (0.0 <= self.entropy_ignore_percentage <= 1.0):
        #     raise ValueError("`entropy_ignore_percentage` must be between 0.0 and 1.0.")
