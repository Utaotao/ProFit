...
def profit_loss_func(outputs, labels, num_items_in_batch=None, threshold_direction: str = "higher", prob_threshold: Union[float, list[float]] = 0.0, shift_labels: Optional[torch.Tensor] = None):
    logits = outputs.get("logits")
    if logits is None:
        return outputs.get("loss", torch.tensor(0.0))
    ...
    shift_labels = shift_labels.view(-1)
    shift_labels = shift_labels.to(logits.device)
    loss = _profit_cross_entropy(logits, shift_labels, num_items_in_batch, prob_threshold, threshold_direction)
    return loss
...
def _profit_cross_entropy(
    ...
    if threshold_direction=="middle":
        mask_condition = ((prob_of_correct_token.detach() < prob_threshold[0]) | (prob_of_correct_token.detach() > prob_threshold[1]))
